...
// Light combinations
#define SET_INDICATORS_COMPONENTS(h, s, v)                      \
	{0, 2, HSV_OVERRIDE_HELP(h, s, v, INDICATOR_BRIGHTNESS)}
#define SET_INDICATORS(hsv) SET_INDICATORS_COMPONENTS(hsv)

#define SET_UNDERGLOW_COMPONENTS(h, s, v)        \
	{2, 12, h, s, v}
#define SET_UNDERGLOW(hsv) SET_UNDERGLOW_COMPONENTS(hsv)

#define SET_NUMPAD(hsv)    \
    {21, 3, hsv}, {33, 3, hsv}, {45, 3, hsv}, {58, 1, hsv}
#define SET_NUMROW(hsv)    \
    {14, 12, hsv}
#define SET_THUMB_CLUSTER_COMPONENTS(h, s, v)    \
    {62, 10, h, s, v}
#define SET_THUMB_CLUSTER(hsv) SET_THUMB_CLUSTER_COMPONENTS(hsv)

#define SET_LAYER_ID(hsv)                       \
    SET_INDICATORS_COMPONENTS(hsv),                       \
    SET_UNDERGLOW_COMPONENTS(hsv),      \
    SET_THUMB_CLUSTER_COMPONENTS(hsv)
 ...
 */
  [_QWERTY] = LAYOUT(
  //,------------------------------------------------.                    ,---------------------------------------------------.
  KC_GRAVE,   KC_1,   KC_2,    KC_3,    KC_4,    KC_5,           LT(_SWITCH,KC_6), KC_7,   KC_8,    KC_9,    KC_0,    KC_MINUS,
  //|------+-------+--------+--------+--------+------|                   |--------+-------+--------+--------+--------+---------|
...
[_LOWER] = LAYOUT(
  ...
  //|------+-------+--------+--------+--------+------|  ===  |   |  ===  |--------+-------+--------+--------+--------+---------|
  _______,  KC_NO,  KC_NO,   KC_NO,   KC_WH_D, KC_PGDN,_______,    _______,KC_HOME, KC_PGDN, KC_PGUP, KC_END, KC_NO,   KC_DEL,
  //|------+-------+--------+--------+--------+------|  ===  |   |  ===  |--------+-------+--------+--------+--------+---------|
 ...
 */
  [_ADJUST] = LAYOUT(
  //,------------------------------------------------.                    ,---------------------------------------------------.
  EEP_RST, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,                   XXXXXXX, KC_KVM1, KC_KVM2, XXXXXXX, XXXXXXX, XXXXXXX,
  //|------+-------+--------+--------+--------+------|                   |--------+-------+--------+--------+--------+---------|
...
[_NUMPAD] = LAYOUT(
  //,------------------------------------------------.                    ,---------------------------------------------------.
  _______, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,                   _______, KC_NLCK, KC_SCRL, KC_PSCR, XXXXXXX, XXXXXXX,
  //|------+-------+--------+--------+--------+------|                   |--------+-------+--------+--------+--------+---------|
...
[_SWITCH] = LAYOUT(
  //,------------------------------------------------.                    ,---------------------------------------------------.
    _______,XXXXXXX,XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,                   XXXXXXX, KC_KVM1, KC_KVM2, XXXXXXX,XXXXXXX, QK_BOOT,
  //|------+-------+--------+--------+--------+------|                   |--------+-------+--------+--------+--------+---------|
...
// QWERTY,
// Light on power LED, thumb row, underglow.
...
// LOWER
// Light on power LED, thumb row, underglow.
const rgblight_segment_t PROGMEM layer_lwr_lights[] = RGBLIGHT_LAYER_SEGMENTS(
	SET_LAYER_ID(HSV_TEAL)
);

// RAISE
// Light on power LED, thumb row, underglow.
const rgblight_segment_t PROGMEM layer_rse_lights[] = RGBLIGHT_LAYER_SEGMENTS(
...
const rgblight_segment_t* const PROGMEM my_rgb_layers[] = RGBLIGHT_LAYERS_LIST(
    layer_qwerty_lights,
	layer_lwr_lights,// overrides layer 1
	layer_rse_lights,
...
void keyboard_post_init_user(void) {
    ...
    // haven't found a way to set this in a more useful way
	rgblight_mode(10);
...
static void print_status_narrow(void) {
    ...
    int mask = rgblight_status.enabled_layer_mask;
    for (int i=sizeof(rgblight_layer_mask_t)*8-1; i>=0; i--) {
        if (mask & (1L<<i))
            oled_write_char('1', true);
        else
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        ...
        case KC_KVM1:
            if (record->event.pressed) {
                SEND_STRING(SS_TAP(X_SCROLLLOCK) SS_DELAY(100) SS_TAP(X_SCROLLLOCK) "1");
            }
            return true;
        case KC_KVM2:
            if (record->event.pressed) {
                SEND_STRING(SS_TAP(X_SCROLLLOCK) SS_DELAY(100) SS_TAP(X_SCROLLLOCK) "2");
            }
            return true;
    }
    return true;
